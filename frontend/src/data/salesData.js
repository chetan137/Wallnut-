/**
 * Wallnut — Mock Sales Register Data
...
function generateTransactions() {
  ...
  const months = [
    { year: 2025, month: 3, label: 'Apr' },
    { year: 2025, month: 4, label: 'May' },
    { year: 2025, month: 5, label: 'Jun' },
  ];
  ...
  for (const monthInfo of months) {
    ...
    for (const stateName of states) {
      ...
      for (const district of stInfo.districts) {
        ...
        const districtMult = 0.7 + rand() * 0.6; // varies per district

        for (const officer of officers) {
          const dealers = stInfo.dealers[officer] || [];

          for (const dealer of dealers) {
            // 1-3 transactions per dealer per month
            ...
            for (let t = 0; t < txnCount; t++) {
              ...
              const rate = Math.round(product.rate * (0.92 + rand() * 0.16)); // ±8% rate variation
              const amount = qty * rate;

              // Outstanding ratio — 15% of dealers have high outstanding
              ...
              const vchNoStr = `${branch}-${String(vchCounter).padStart(3, '0')}/25-26`;
...
export const salesData = generateTransactions();

// Extract unique districts
const _districts = new Set();
const _stateOfDistrict = {};
salesData.forEach(r => {
  _districts.add(r.areaCity);
  _stateOfDistrict[r.areaCity] = r.state;
});
export const allDistricts = [..._districts];
export const districtToState = _stateOfDistrict;

// Extract unique states
export const allStates = Object.keys(stateData);

// Extract unique officers with their district + state
const _officerMap = {};
salesData.forEach(r => {
...
export const allSalesOfficers = Object.values(_officerMap);

// Extract unique dealers with their officer + district + state
const _dealerMap = {};
salesData.forEach(r => {
...
export const allDealers = Object.values(_dealerMap);
